...
class BVInfoCallback {
	...
	public function servicesInfo(&$info) {
		...
		$info['brand'] = $bvinfo->getTransient($this->bvmain->brand_transient);
	}
	...
	public function getLPConfig() {
		...
		$config = array();
		$bvinfo = $bvcb->bvmain->info;
		$config['mode'] = intval($bvinfo->getOption('bvlpmode') ?: 1);
		$config['blacklisted_ips'] = $bvinfo->getOption('bvlpblacklistedips') ?: array();
		$config['whitelisted_ips'] = $bvinfo->getOption('bvlpwhitelistedips') ?: array();
		$config['captcha_limit'] = intval($bvinfo->getOption('bvlpcaptchalimit') ?: 3);
		$config['temp_block_limit'] = intval($bvinfo->getOption('bvlptempblocklimit') ?: 6);
		$config['block_all_limit'] = intval($bvinfo->getOption('bvlpblockAllLimit') ?: 100);
		return $config;
	}
	...
	public function getFWConfig() {
		...
		$config = array();
		$bvinfo = $bvcb->bvmain->info;
		$config['mode'] = intval($bvinfo->getOption('bvfwmode') ?: 1);
		$config['blacklisted_ips'] = $bvinfo->getOption('bvfwblacklistedips') ?: array();
		$config['whitelisted_ips'] = $bvinfo->getOption('bvfwwhitelistedips') ?: array();
		$config['disabled_rules'] = $bvinfo->getOption('bvfwdisabledrules') ?: array();
		$config['rules_mode'] = intval($bvinfo->getOption('bvfwrulesmode') ?: 1);
		return $config;
	}
