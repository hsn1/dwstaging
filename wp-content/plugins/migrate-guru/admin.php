...
class MGAdmin {
	...
	public function initHandler() {
		...
		if (array_key_exists('bvnonce', $_REQUEST) &&
			...
			if (array_key_exists('redirect', $_REQUEST)) {
				...
				wp_redirect($this->bvmain->authenticatedUrl('/migration/'.$location));
				exit();
			}
		}
	...
	public function menu() {
		$brand = $this->bvmain->info->getTransient($this->bvmain->brand_transient);
		if ($brand) {
			$bname = (array_key_exists('hide', $brand) ? false : $brand['menuname']);
		} else {
			$bname = $this->bvmain->brandname;
		}
		if ($bname) {
	...
	public function mgsecAdminMenu() {
...
?>
			<div id="message" class="updated" style="padding: 8px; font-size: 16px; background-color: #dff0d8">
						...
						&nbsp;&nbsp;&nbsp;<b>Almost Done:</b> Activate your Migrate Guru account to secure your site.
			</div>
<?php
		}
	}
