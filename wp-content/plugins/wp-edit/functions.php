...
$plugin_options_fonts = get_option('wp_edit_fonts');
...
if($plugin_options_global['disable_admin_links'] != 1) {
	...
	function wp_edit_admin_bar_links() {
		...
		$wp_admin_bar->add_menu( array(
			...
			'parent' => 'jwl_links',
			'title' => __('Fonts',''),
			'href'  => $path.'admin.php?page=wp_edit_options&tab=fonts',
			'meta'  => array(
				'title' => __('Fonts','')
			),
		));
		$wp_admin_bar->add_menu( array(
			'id'    => 'jwl_admin_widgets',
			'parent' => 'jwl_links',
			'title' => __('Widgets','wp_edit_langs'),
			'href'  => $path.'admin.php?page=wp_edit_options&tab=widgets',
			'meta'  => array(
				'title' => __('Widgets','wp_edit_langs')
			),
		));
...
// Disable wpautop
if(!empty($plugin_options_posts['disable_wpautop']) && $plugin_options_posts['disable_wpautop'] != '') {
	
	if ( ! class_exists( 'wp_edit_Toggle_wpautop' ) ) {
	
		/**
		 * JWL_Toggle_wpautop class.
		 */
		class wp_edit_Toggle_wpautop {
			...
			function admin_init() {
				...
				add_settings_section( 'jwl_toggle_wpautop_settings_section', 'Toggle wpautop', array( &$this, 'settings_section_text' ), 'writing' );
			...
			function post_submitbox_misc_actions() {
				...
				<div class="misc-pub-section jwl-wpautop">
					<span><?php _e('Disable wpautop:', 'wp_edit_langs'); ?></span> <input type="checkbox" name="_jwl_disable_wpautop" id="_jwl_disable_wpautop" <?php checked( get_post_meta( $post->ID, '_jwl_disable_wpautop', true ) ); ?> /> <span style="float:right; display: block;"><a href="http://codex.wordpress.org/Function_Reference/wpautop" target="_blank">?</a>
				</div>
				<?php
			}
	...
	$wp_edit_Toggle_wpautop = new wp_edit_Toggle_wpautop();
}
