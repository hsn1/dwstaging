/*! jquery-locationpicker - v0.1.15 - 2017-10-17 */
(function(jQuery) {
    "use strict";
    ...
    jQuery.fn.popmake.methods = {
        init: function(options) {
            return this.each(function() {
                var $this = jQuery(this), settings = jQuery.extend(true, {}, jQuery.fn.popmake.defaults, $this.data("popmake"), options);
                ...
                $this.data("popmake", settings).on("popmakeBeforeOpen.hide_popup", function() {
                    jQuery(this).css({
                        visibility: "visible"
                    }).hide().addClass(settings.container.active_class);
                    ...
                    jQuery("iframe", $this).filter('[src*="youtube"],[src*="vimeo"]').each(function() {
                        var src = jQuery(this).attr("src").replace("autoplay=1", "1=1");
        ...
        setup_close: function() {
            var $this = jQuery(this), settings = $this.data("popmake"), $overlay = jQuery("#" + settings.overlay.attr.id), $close = jQuery("." + settings.close.attr.class, $this);
            $close.off("click.popmake").on("click.popmake", function(e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            if (settings.meta.close.esc_press || settings.meta.close.f4_press) {
                jQuery(window).off("keyup.popmake").on("keyup.popmake", function(e) {
            ...
            if (settings.meta.close.overlay_click) {
                $overlay.off("click.popmake").on("click.popmake", function(e) {
                    console.log("bbbb");
                    e.preventDefault();
                    e.stopPropagation();
        ...
        open: function(callback) {
            var $this = jQuery(this), settings = $this.data("popmake");
            $this.trigger("popmakeBeforeOpen").popmake("animate", settings.meta.display.animation_type, function() {
                $this.addClass("active").trigger("popmakeAfterOpen");
        ...
        close: function() {
            return this.each(function() {
                var $this = jQuery(this), settings = $this.data("popmake");
                $this.trigger("popmakeBeforeClose").fadeOut(settings.close.close_speed, function() {
                    $this.removeClass("active").trigger("popmakeAfterClose");
                });
                return this;
            });
        },
        ...
        reposition: function(callback) {
            ...
            var $this = jQuery(this), settings = $this.data("popmake"), display = settings.meta.display, location = display.location, reposition = {
                my: "",
                at: ""
            }, opacity = false;
            ...
            $this.removeClass("responsive size-nano size-micro size-tiny size-small size-medium size-normal size-large size-xlarge fixed custom-position").addClass("size-" + settings.meta.display.size);
            ...
            $this.addClass("custom-position").position(reposition).trigger("popmakeAfterReposition");
            if (opacity) {
                $this.css({
                    opacity: opacity
                }).hide();
        ...
        retheme: function(theme) {
            ...
            var $this = jQuery(this), settings = $this.data("popmake"), $overlay = jQuery("#" + settings.overlay.attr.id), $container = $this, $title = jQuery("." + settings.title.attr.class, $container), $content = jQuery("> ." + settings.content.attr.class, $container), $close = jQuery("> ." + settings.close.attr.class, $container), container_inset, close_inset;
            if (theme === undefined) {
    ...
    jQuery.fn.popmake.cookie = {
        ...
        process: function(key, value, expires, path) {
            var result = key ? undefined : {}, t = new Date(), cookies = document.cookie ? document.cookie.split("; ") : [], parts, name, cookie, i, l;
            if (value !== undefined && !jQuery.isFunction(value)) {
                switch (typeof expires) {
                  case "number":
                    t.setTime(+t + expires * 864e5);
                    expires = t;
                    break;

                  case "string":
                    t.setTime(jQuery.fn.popmake.utilities.strtotime("+" + expires) * 1e3);
                    expires = t;
                    break;
                }
                document.cookie = [ jQuery.fn.popmake.cookie.encode(key), "=", jQuery.fn.popmake.cookie.stringifyCookieValue(value), expires ? "; expires=" + expires.toUTCString() : "", path ? "; path=" + path : "" ].join("");
                return;
            }
    ...
    jQuery.fn.popmake.utilities = {
        convert_hex: function(hex, opacity) {
            hex = hex.replace("#", "");
            var r = parseInt(hex.substring(0, 2), 16), g = parseInt(hex.substring(2, 4), 16), b = parseInt(hex.substring(4, 6), 16), result = "rgba(" + r + "," + g + "," + b + "," + opacity / 100 + ")";
            return result;
        },
        ...
        throttle: function(callback, threshold) {
            var suppress = false, clear = function() {
                suppress = false;
            };
            return function() {
                if (!suppress) {
        ...
        getXPath: function(element) {
            var path = [], current, id, classes, tag, eq;
        ...
        strtotime: function(text, now) {
                ...
                if (match[1] > 1901) {
                    ...
                    switch (match[2]) {
                      case "-":
                        ...
                        year = match[1] >= 0 && match[1] <= 38 ? +match[1] + 2e3 : match[1];
                        return new Date(year, parseInt(match[3], 10) - 1, match[5], match[6] || 0, match[7] || 0, match[8] || 0, match[9] || 0) / 1e3;
                      ...
                      case ".":
                        ...
                        if (match[5] < 60 && !match[6]) {
                            ...
                            today = new Date();
                            return new Date(today.getFullYear(), today.getMonth(), today.getDate(), match[1] || 0, match[3] || 0, match[5] || 0, match[9] || 0) / 1e3;
                        }
                        return fail;
                      ...
                      case "/":
                        ...
                        year = match[5] >= 0 && match[5] <= 38 ? +match[5] + 2e3 : match[5];
                        return new Date(year, parseInt(match[1], 10) - 1, match[3], match[6] || 0, match[7] || 0, match[8] || 0, match[9] || 0) / 1e3;
                      ...
                      case ":":
                        ...
                        today = new Date();
                        return new Date(today.getFullYear(), today.getMonth(), today.getDate(), match[1] || 0, match[3] || 0, match[5] || 0) / 1e3;
                    }
                }
            }
            if (text === "now") {
            ...
            date = now ? new Date(now * 1e3) : new Date();
            days = {
                sun: 0,
                mon: 1,
                tue: 2,
    ...
    jQuery.fn.popmake.animations = {
        ...
        slide: function(callback) {
            var $this = jQuery(this).show(0).css({
                opacity: 0
            }), settings = $this.data("popmake"), speed = settings.meta.display.animation_speed / 2e3, start = $this.popmake("animation_origin", settings.meta.display.animation_origin);
            jQuery("html").css("overflow-x", "hidden");
            $this.position(start).css({
                opacity: 1
            }).popmake("animate_overlay", "fade", speed * 1e3, function() {
        ...
        fade: function(callback) {
            var $this = jQuery(this).show(0).css({
                opacity: 0
            }), settings = $this.data("popmake"), speed = settings.meta.display.animation_speed / 2e3;
            $this.popmake("animate_overlay", "fade", speed * 1e3, function() {
        ...
        fadeAndSlide: function(callback) {
            ...
            jQuery("html").css("overflow-x", "hidden");
            $this.position(start).popmake("animate_overlay", "fade", speed * 1e3, function() {
                $this.popmake("reposition", function(position) {
                    TweenLite.to($this, speed, jQuery.extend(position, {
                        opacity: 1,
                        onComplete: function() {
        ...
        grow: function(callback) {
            var $this = jQuery(this).show(0).css({
                opacity: 0
            }), settings = $this.data("popmake"), speed = settings.meta.display.animation_speed / 2e3, origin = settings.meta.display.animation_origin;
        ...
        growAndSlide: function(callback) {
            var $this = jQuery(this).show(0).css({
                opacity: 0
            }), settings = $this.data("popmake"), speed = settings.meta.display.animation_speed / 2e3, origin = settings.meta.display.animation_origin, start = $this.popmake("animation_origin", origin);
            jQuery("html").css("overflow-x", "hidden");
            $this.position(start);
            TweenLite.to($this, 0, {
                scale: 0,
    ...
    jQuery(document).ready(function() {
        jQuery(".popmake").popmake().each(function() {
            var $this = jQuery(this), settings = $this.data("popmake"), click_open = settings.meta.click_open, trigger_selector = ".popmake-" + settings.id + ", .popmake-" + settings.slug, admin_debug = settings.meta.admin_debug, auto_open = settings.meta.auto_open, noCookieCheck;
            if (click_open !== undefined && click_open.extra_selectors !== "") {
                trigger_selector += ", " + click_open.extra_selectors;
            }
            jQuery(trigger_selector).css({
                cursor: "pointer"
            });
            ...
            if (auto_open !== undefined && auto_open.enabled) {
                ...
                $this.on("popmakeSetCookie", function() {
                    if (auto_open.cookie_time !== "" && noCookieCheck()) {
                        jQuery.pm_cookie("popmake-auto-open-" + settings.id + "-" + auto_open.cookie_key, true, auto_open.cookie_time, auto_open.cookie_path);
                    }
                });
