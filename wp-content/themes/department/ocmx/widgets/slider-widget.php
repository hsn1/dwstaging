		...
		<div class="slider clearfix" id="slider-widget">
			<ul>
				<?php while ( $slider_posts->have_posts() ) : $slider_posts->the_post();
					...
					if( isset( $sliderbg_attributes[ 'repeat' ] ) && !strpos( $sliderbg_attributes[ 'repeat' ] , 'no' ) ) {
						$cover_css = '';
					} else {
						$cover_css = ' background-size: cover;';
					}
					...
					<li class="<?php if($slider_posts->current_post != 0 ) : ?>no_display<?php else : ?>active<?php endif; ?> slide" style="background: url(<?php echo $sliderbg; ?>) <?php if(!empty($sliderbg_attributes)) echo implode(" ", $sliderbg_attributes); ?>; <?php echo $cover_css; ?>">
							...
							<div class="slider-copy <?php // opacity_zero; ?> ">
								...
								<?php if($thecontent !='') : ?>
									<p <?php if($textcolour != '') : ?>style="color: <?php echo $textcolour; ?>;"<?php endif; ?>>
										<?php echo strip_tags( $thecontent ); ?>
									</p>
								<?php endif; ?>
								<?php if($link !='') : ?>
