...
function get_obox_media($args){
	...
	$self_hosted_video = get_post_meta($postid, "video_hosted", true);
        $youtube_link_fr="https://www.youtube.com/watch?v=LFOKTfPe4l8";
        $youtube_link_en="https://www.youtube.com/watch?v=LFOKTfPe4l8";
	...
	if (function_exists("has_post_thumbnail") && has_post_thumbnail($postid)) : // WordPress Thumbnail overrides everything
            if($postid == 617) :
                $img=get_the_post_thumbnail($postid, $resizer);
                $image= do_shortcode('[su_lightbox src="https://www.youtube.com/watch?v=LFOKTfPe4l8" class="popbox"]'.$img.'[/su_lightbox]');
	...
	elseif($hide_href == false && isset($image)) : // If we got an image and we want to link it
		$html = "<a href=\"#\" $href_class>$image</a>";
