...
class Shared {
	 ...
	 *
	 * @param  array $query_vars List of WordPress native query vars.
	 * @return object the query_vars object with the extended
	 */
	public function kinsta_api_query_vars( $query_vars ) {
		if ( isset( $_GET['zrm3ajc4yvmdbwc78k7c8der3uu47ah6fmx9648g_kinsta_site_api'] ) ) { // WPCS: CSRF ok.
			$query_vars[] = 'zrm3ajc4yvmdbwc78k7c8der3uu47ah6fmx9648g_kinsta_site_api';
		}
		if ( isset( $_GET['d2ksbky5nbhjd7zzbu8gtymzc4mz43tcypykubxt_kinsta_site_api'] ) ) { // WPCS: CSRF ok.
			$query_vars[] = 'd2ksbky5nbhjd7zzbu8gtymzc4mz43tcypykubxt_kinsta_site_api';
		}
		return $query_vars;
	}
	 ...
	 * @return void
	 */
	public function kinsta_api_parse_request( $wp ) {
		if ( array_key_exists( 'zrm3ajc4yvmdbwc78k7c8der3uu47ah6fmx9648g_kinsta_site_api', $wp->query_vars ) ) {
			if ( 'cdninfo' === $wp->query_vars['zrm3ajc4yvmdbwc78k7c8der3uu47ah6fmx9648g_kinsta_site_api'] ) {
				include 'api/cdn-info.php';
			}
			return;
		}
		if ( array_key_exists( 'd2ksbky5nbhjd7zzbu8gtymzc4mz43tcypykubxt_kinsta_site_api', $wp->query_vars ) ) {
			if ( 'siteinfo' === $wp->query_vars['d2ksbky5nbhjd7zzbu8gtymzc4mz43tcypykubxt_kinsta_site_api'] ) {
				include 'api/site-info.php';
			}
			return;
		}
	}
	...
	public function modify_admin_footer_text() {
		...
		return '<span id="footer-thankyou">' . sprintf( __( 'Thanks for creating with %1$s and hosting with %2$s', 'kinsta-mu-plugins' ), '<a href="https://wordpress.org/">WordPress</a>', '<a href="https://kinsta.com" target="_blank">Kinsta</a>' ) . '</span>';
	}
}
